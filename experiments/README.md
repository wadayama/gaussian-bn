...
Eight self-contained experiments built on the `gaussian_bn` public API. Each
...
```
experiments/
├── exp{1,2,3,5,6,7,8,9}_*.py        # run scripts
├── results/exp{1,2,3,5,6,7,8,9}.json  # numerical results (committed source of truth)
├── results/lds_*.dat            # pgfplots data for the paper LDS figure (exp8)
├── results/calib_*.dat          # pgfplots data for the paper calibration figure (exp9)
